...
4. **Taobao login** (first time setup):
   ```bash
   python ai_scraper.py --login
   ```

## Command Line Options

```bash
python ai_scraper.py [OPTIONS]
```
...
```bash
python upload_to_knack.py --dry-run    # Preview first
python upload_to_knack.py --sync-media # Upload + sync images
```
...
```bash
python upload_to_knack.py --help
