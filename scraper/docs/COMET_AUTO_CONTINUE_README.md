...
# Or with arguments
python3 comet_simple_continue.py --timeout 60 --message "keep going"
python3 comet_simple_continue.py -t 90 -e  # Just press Enter
```
...
# With options
python3 comet_auto_continue.py --interval 20 --timeout 60 --prompt "continue"

# Test mode (won't actually send input)
python3 comet_auto_continue.py --dry-run
```
...
3. If Comet needs specific input, customize the message:
   ```bash
  python3 comet_simple_continue.py -m "yes, continue to the next variant"
   ```
