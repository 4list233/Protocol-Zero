...
const MANIFEST_PATH = path.join(repoRoot, 'shared', 'data', 'products_manifest.json');
const MEDIA_DIR = path.join(repoRoot, 'shared', 'media');
...
async function uploadImageToNotion(imagePath, productId) {
  ...
  if (!path.isAbsolute(imagePath)) {
    ...
    const possiblePaths = [
      path.resolve(repoRoot, 'shop', 'public', 'images', imagePath),
      path.resolve(repoRoot, 'scraper', 'ai_scraper_output', 'media', imagePath),
      path.resolve(repoRoot, 'scraper', 'media', imagePath),
      path.resolve(repoRoot, 'scraper', imagePath),
  ...
  try {
    ...
    const publicDir = path.resolve(repoRoot, 'shop', 'public', 'images');
